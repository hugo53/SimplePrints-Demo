...
- (void)collectionView:(PSTCollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [_browser hideAddSelectedPhotoBtn];
    [_browser showToolbar];
    [_browser hideGrid];
    _browser.isDisplayingFullPhoto = YES;
    
    if (!_browser.allowLeftBtnOnNavigation) {
        // Turn on left button if go to full image
        [_browser enableLeftNavBtn:YES];
    }
    
    if (_browser.allowRightBtnOnNavigation) {
        // Turn off right button if go to full imgage
        [_browser enableRightNavBtn:NO];
    }
