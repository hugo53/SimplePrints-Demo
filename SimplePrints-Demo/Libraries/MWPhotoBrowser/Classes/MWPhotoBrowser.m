...
- (void)_initialisation {
    ...
    _isDisplayingFullPhoto = NO;
    _amountOfSelectedPhoto = 0;
    _showAddSelectedPhotoBtn = YES;
    _allowRightBtnOnNavigation = NO;
    _allowLeftBtnOnNavigation = NO;
...
- (void)setNavBarAppearance:(BOOL)animated {
    ...
    if(_allowLeftBtnOnNavigation) {
        [self enableLeftNavBtn:YES];
    }
    
    if (_allowRightBtnOnNavigation) {
        [self enableRightNavBtn:YES];
    }
}

- (void) enableRightNavBtn:(BOOL) enabled {
    if (enabled) {
        float shiftLeftOffset = 10.0;
        // Set left button
        UIButton *rightBtn = [[UIButton alloc] initWithFrame: CGRectMake(0, 0, 44.0f, 30.0f)];
        [rightBtn setImage:[UIImage imageNamed:@"add"]  forState:UIControlStateNormal];
        [rightBtn addTarget:self action:@selector(didPressOnRightBtn) forControlEvents:UIControlEventTouchUpInside];
        rightBtn.imageEdgeInsets = UIEdgeInsetsMake(0.0, shiftLeftOffset, 0, -shiftLeftOffset);
...
- (void) enableLeftNavBtn:(BOOL) enabled {
    if (enabled) {
...
- (void)setPhotoSelected:(BOOL)selected atIndex:(NSUInteger)index {
    if (_displaySelectionButtons) {
        if ([self.delegate respondsToSelector:@selector(photoBrowser:photoAtIndex:selectedChanged:)]) {
            ...
            if (selected) {
                _amountOfSelectedPhoto++;
//                MWPhoto * photo = [self photoAtIndex:index];
//                NSLog(@"Did select photo at link: %@",[photo.photoURL absoluteString]);
            }else {
                _amountOfSelectedPhoto--;
            }
            [self updateNavigation];
...
- (void)updateNavigation {
    ...
    if (!_showAddSelectedPhotoBtn) {
        self.title = NSLocalizedString(@"Selected Photos", nil);
    }else {
        if (_amountOfSelectedPhoto == 0) {
            self.title = NSLocalizedString(@"Select Photos", nil);
        }else if (_amountOfSelectedPhoto > 0) {
            self.title = [NSString stringWithFormat:@"%d Selected", _amountOfSelectedPhoto];
        }else{}
    }
...
- (void)showGrid:(BOOL)animated {
    ...
    CGRect gridViewFrame = self.view.bounds;

    if (_showAddSelectedPhotoBtn) {
        if (!addSelectedPhotoBtn) {
            addSelectedPhotoBtn = [[UIButton alloc] init];
            
//            [addSelectedPhotoBtn setTitle:@"Add Selected Photos" forState:UIControlStateNormal];
//            [addSelectedPhotoBtn setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
            [addSelectedPhotoBtn setImage:[UIImage imageNamed:@"addSelectedPhotoImg"] forState:UIControlStateNormal];
            addSelectedPhotoBtn.frame = CGRectMake(self.view.bounds.origin.x,
                                                   self.view.bounds.size.height-50,
                                                   self.view.bounds.size.width, 50);
            [addSelectedPhotoBtn addTarget:self action:@selector(addSelectedPhoto) forControlEvents:UIControlEventTouchUpInside];
            [self enableAddSelectedPhotoBtn:NO];
        }
        
        [self.view addSubview:addSelectedPhotoBtn];
        gridViewFrame.size.height -= addSelectedPhotoBtn.frame.size.height;
    }
    ...
    _gridController.view.frame = gridViewFrame;
    _gridController.view.frame = CGRectOffset(_gridController.view.frame,(self.startOnGrid ? -1 : 1) * self.view.bounds.size.width, 0);
...
//        newPagingFrame = CGRectOffset(newPagingFrame, 0, (self.startOnGrid ? 1 : -1) * newPagingFrame.size.height);
        newPagingFrame = CGRectOffset(newPagingFrame,(self.startOnGrid ? 1 : -1) * newPagingFrame.size.width,0);
...
- (void)hideGrid {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        tmpGridController.view.frame = CGRectOffset(self.view.bounds, (self.startOnGrid ? -1 : 1) * self.view.bounds.size.height,0);
...
- (void) didPressOnBackBtn {
    if (_isDisplayingFullPhoto) {
        _isDisplayingFullPhoto = NO;
        addSelectedPhotoBtn.hidden = NO;
        if (_toolbar) {
            [_toolbar removeFromSuperview];
        }
        
        [self enableRightNavBtn:_allowRightBtnOnNavigation];
        [self enableLeftNavBtn:_allowLeftBtnOnNavigation];
...
- (void) didPressOnRightBtn {
    if ([self.delegate respondsToSelector:@selector(addMoreImageIntoPhotoBrowser:)]) {
        [self.delegate addMoreImageIntoPhotoBrowser:self];
    }
}

- (void) enableAddSelectedPhotoBtn:(BOOL) enabled {
    addSelectedPhotoBtn.enabled = enabled;
}

- (void) hideAddSelectedPhotoBtn {
    if (addSelectedPhotoBtn) {
        addSelectedPhotoBtn.hidden = YES;
    }
}
...
- (void) addSelectedPhoto {
    if ([self.delegate respondsToSelector:@selector(addSelectedPhotoFromPhotoBrowser:)]) {
        [self.delegate addSelectedPhotoFromPhotoBrowser:self];
    }
}
